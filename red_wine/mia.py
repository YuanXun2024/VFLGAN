...
def main():
    ...
    for _ in range(5):
        ...
        # print(test_set)
        for i in train_set:
            shadow_dir = shadow_path + str(i) + "/data_onehot.npy"
            data = np.load(shadow_dir)
            ano_dir = ano_path + str(i) + "/data_onehot.npy"
            data_ano = np.load(ano_dir)
            for j in range(1):   
                assert(len(data[1599*j:1599*(j+1), :]) != 0)        
                F = Features(data[1599*j:1599*(j+1), :])
                corr = F.extract_corr()
                feature_shadow += [corr]          

                F_a = Features(data_ano[1599*j:1599*(j+1), :])
