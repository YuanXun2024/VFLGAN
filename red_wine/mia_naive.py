...
def main():
    ...
    # ano_path = "generated_data/WGAN_vfl_LOO_non_epsilon_1235/"
    ...
    scores = []
    with open('data/winequality-red.json', 'r') as fcc_file:
    ...
    for _ in range(5):
        ...
        # print(test_set)
        for i in train_set:
            shadow_dir = shadow_path + str(i) + "/redwine.csv"
            data = pd.read_csv(shadow_dir)
            ano_dir = ano_path + str(i) + "/redwine.csv"
            data_ano = pd.read_csv(ano_dir)
            for j in range(1):  
                # assert(len(data[42960*j:42960*(j+1), :]) != 0)        
                feature_shadow += [F.extract(data.iloc[:1599, :])]          
                feature_ano += [F.extract(data_ano.iloc[:1599, :])]
