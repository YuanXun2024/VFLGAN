...
torch.cuda.set_device('cuda:'+str(7))
...
parser.add_argument("--n_steps", type=int, default=100, help="number of steps of epoch")
...
parser.add_argument("--sigma", type=float, default=2.348, help="interval betwen image samples")
...
def train(model_list, optimizer_list, dataset, data, param_path="params/WGAN_centric_shadow/0/"):
    ...
    def func(grad):
        ...
        grad = grad*clip_coef + clip_norm * SENSITIVITY * opt.sigma * torch.randn([1]).cuda().float()
        return grad
...
def initialization(seed, delet_target=None):
    ...
    return model_list, optimizer_list, dataset, data


if __name__ == '__main__':
    target = 1235  
    print('delet target:', target if target else 'shadow') 
    print('5-DP')
    ...
    for i in tqdm(range(25)):
        for j in range(4):
            ...
            if target is not None:
                param_path = 'params/DP_WGAN_vfl_LOO_epsilon_5_' + str(target) + '/' + str(seed) + '/'
            else:
                param_path = 'params/DP_WGAN_vfl_shadow_epsilon_5' + '/' + str(seed) + '/'
            if j == 0:
