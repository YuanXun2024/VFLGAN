...
def generate_vfl_gumbel(param_path, data_path, size=1599, delet_target=None):
    ...
    data[:, 11] = data[:, 11] + data[:, 12]*2 + data[:, 13]*3 + data[:, 14]*4 + data[:, 15]*5 + data[:, 16]*6 + data[:, 17]*7 + 2
    data_dir = data_path + "/data.npy"
    np.save(data_dir, data[:, :12])
    


def main(param_path, data_path, size=4898, mode='centric', delet_target=None):
    generate_vfl_gumbel(param_path, data_path, size=size, delet_target=delet_target)


if __name__ == "__main__":
    for i in tqdm(range(100)):
        param_path = "params/WGAN_vfl_LOO_non_epsilon_1034/" + str(i)
        data_path = "generated_data/WGAN_vfl_LOO_non_epsilon_1034/" + str(i)
        os.makedirs(data_path, exist_ok=True)
        main(param_path, data_path, delet_target=1034)
