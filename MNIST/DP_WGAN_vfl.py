...
parser.add_argument("--sigma", type=float, default=0.802, help="interval betwen image samples")
...
os.makedirs("./generated_data/DP_vfl", exist_ok=True)
os.makedirs("./params/DP_vfl", exist_ok=True)

class MyDataset(Dataset):
    def __init__(self, imgpath='./data/mnist/MNIST/raw/train-images-idx3-ubyte.gz', transform=None):
        with gzip.open(imgpath, 'rb') as img:
            x_train = np.frombuffer(img.read(), np.uint8, offset=16).reshape([-1, 28, 28])
...
class DiscriminatorClient(nn.Module):
    def __init__(self):
        ...
        self.model = nn.Sequential(
            nn.Linear(int(np.prod(img_shape)), 512, bias=False),
...
mnist_data = MyDataset(transform=transforms.Compose(
...
def main():
    ...
    for epoch in tqdm(range(opt.n_epochs)):
        for i in range(937):
            index = random.sample(range(60000), opt.batch_size)
            imgs = mnist_data.__getitem__(index=index)
            ...
            if i % opt.n_critic == 0:
                ...
                if batches_done % opt.sample_interval == 0:
                    ...
                    save_image(fake_imgs.data[:25], "generated_data/DP_vfl/%d.png" % batches_done, nrow=5, normalize=True)

                batches_done += opt.n_critic

        if (epoch + 1) % 10 == 0:
            torch.save(G_1.state_dict(), "params/DP_vfl/G_1_%d.pth" % (epoch + 1))
            torch.save(G_2.state_dict(), "params/DP_vfl/G_2_%d.pth" % (epoch + 1))
           

def inference(epoch=26):
    G_1 = Generator().cuda()
    G_2 = Generator().cuda()

    name_1 = 'G_1_' + str(epoch*10) + '.pth'
    name_2 = 'G_2_' + str(epoch*10) + '.pth'
    ...
    fake_imgs = torch.cat((fake_imgs_client_1, fake_imgs_client_2), dim=2)
    save_image(fake_imgs.data[:25], "generated_data/DP_VFLGAN.png", nrow=5, normalize=True)
