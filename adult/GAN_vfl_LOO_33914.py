...
torch.cuda.set_device('cuda:'+str(1))
...
def train(model_list, optimizer_list, dataloader, data_new, param_path="params/WGAN_centric_shadow/0/"):
    ...
    for epoch in range(opt.n_epochs):
        for i, (imgs, _) in enumerate(dataloader):
            ...
            # Adversarial loss
            lambda_dp = 1.0
            d_loss = -torch.mean(real_validity) - lambda_dp * torch.mean(
                real_private_validity_1) - lambda_dp * torch.mean(real_private_validity_2) + \
                     torch.mean(fake_validity) + lambda_dp * torch.mean(
                fake_private_validity_1) + lambda_dp * torch.mean(fake_private_validity_2) + \
            ...
            if i % opt.n_critic == 0:
                ...
                g_loss = -torch.mean(fake_validity) - lambda_dp * (
                            torch.mean(fake_private_validity_1) + torch.mean(fake_private_validity_2))
                g_loss.backward()
...
def initialization(seed, delet_target=None):
    ...
    return model_list, optimizer_list, dataloader, data_new
...
if __name__ == '__main__':
    ...
    for i in tqdm(range(25)):
        for j in range(4):
            ...
            param_path = 'params/WGAN_vfl_LOO_' + str(target) + '/' + str(seed) + '/'
            if j == 0:
